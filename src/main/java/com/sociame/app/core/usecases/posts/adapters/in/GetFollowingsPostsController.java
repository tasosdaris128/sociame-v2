...
public class GetFollowingsPostsController {
    ...
    public GetFollowingsPostsResponse getFollowingsPosts(Authentication authentication) {
        UserDetailsImpl principal = (UserDetailsImpl) authentication.getPrincipal();

        log.info("Request principal for followings: {}", principal);

        return useCase.handleCommand(new GetFollowingsPostsCommand(principal.getUsername()));
    }
